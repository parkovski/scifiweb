...
pub trait MailboxAccessor<'a>: Clone + Send + Sync {
  fn create_mailbox(&self, owner: Target, name: &str, message_limit: MessageLimit, thread_limit: u32)
  ...
  fn get_all_mailboxes(&self, owner: Target)
    -> MailboxFuture<'a, Vec<Mailbox>>;

  fn delete_mailbox_for_owner(&self, owner: Target, name: &str)
    -> MailboxFuture<'a, ()>;
...
pub trait MessageThreadAccessor<'a>: Clone + Send + Sync {
  ...
  fn get_thread_by_id(&self, id: u64) -> MailboxFuture<'a, MessageThread> {
    self.get_threads_by_id(&[id], true)
      .and_then(move |mut vec|
        vec.drain(..).nth(0).ok_or_else(|| MailboxError::not_found("id", id.to_string().as_str()))
      ).into_box()
  }
