...
impl<'a> MailboxAccessor<'a> for Arc<MemoryAccessor> {
  ...
  ) -> MailboxFuture<'a, Mailbox>
  {
    ...
    self.mailbox_cache.write(move |result| result.unpoisoned().put_mailbox(mailbox)).into_box()
  }

  fn get_mailbox_for_owner(&self, owner: Target, name: &str)
    -> MailboxFuture<'a, Mailbox>
  {
    let name = name.to_owned();
    self.mailbox_cache.read(move |result| {
      result.unpoisoned()
  ...
  fn delete_mailbox_for_owner(&self, owner: Target, name: &str) -> MailboxFuture<'a, ()> {
    let name = name.to_owned();
    let this = self.clone();
    self.mailbox_cache.write(move |result| {
      result.unpoisoned().delete_mailbox_for_owner(owner, name.as_str())
    })
    .and_then(move |ids| this.delete_threads(&ids)).into_box()
  }

  fn delete_mailbox_by_id(&self, id: u64) -> MailboxFuture<'a, ()> {
    let this = self.clone();
    self.mailbox_cache.write(move |result| {
      result.unpoisoned().delete_mailbox_by_id(id)
    })
    .and_then(move |ids| this.delete_threads(&ids)).into_box()
  }
...
impl<'a> MessageThreadAccessor<'a> for Arc<MemoryAccessor> {
  fn create_thread(&self, mailbox_id: u64, sender: Target)
    -> MailboxFuture<'a, MessageThread>
  {
    ...
    let this = self.clone();
    self.message_thread_cache.write(move |result| {
      result.unpoisoned().put_thread(thread.clone())
    })
    .and_then(move |thread| this.mailbox_cache.write(move |result| {
      match result.unpoisoned().get_mailbox_by_id_mut(mailbox_id) {
        Some(mut mailbox) => mailbox.thread_ids_mut().push(thread.id()),
        None => {
          debug!("Thread {} still exists even though mailbox was not found", thread.id);
  ...
  fn get_threads_by_id(&self, ids: &[u64], missing_is_error: bool) -> MailboxFuture<'a, Vec<MessageThread>> {
    let ids = Vec::from(ids);
    self.message_thread_cache.read(move |result| {
  ...
  fn get_all_threads(&self, mailbox_id: u64) -> MailboxFuture<'a, Vec<MessageThread>> {
    let this = self.clone();
    self.mailbox_cache.read(move |result|
      result.unpoisoned().get_mailbox_by_id(mailbox_id)
        .ok_or_else(|| MailboxError::not_found("mailbox id", mailbox_id))
    )
    .and_then(move |mailbox| this.message_thread_cache.write(move |result|
      Ok(result
        .unpoisoned()
  ...
  fn get_threads_for_sender(&self, mailbox_id: u64, sender: Target)
    -> MailboxFuture<'a, Vec<MessageThread>>
  {
    ...
    .and_then(move |mailbox| this.message_thread_cache.read(move |result|
      Ok(result
        .unpoisoned()
        .get_threads_by_id(mailbox.thread_ids())
        .into_iter()
  ...
  fn delete_thread(&self, id: u64) -> MailboxFuture<'a, ()> {
    let this = self.clone();
    self.message_thread_cache.write(move |result| {
      Ok(result.unpoisoned().delete_threads(&[id]))
    })
    .and_then(move |ids| this.message_cache.write(move |result|
      Ok(result.unpoisoned().delete_messages(&ids))
    )).into_box()
  }
  ...
  fn delete_threads(&self, ids: &[u64]) -> MailboxFuture<'a, ()> {
    ...
    let this = self.clone();
    self.message_thread_cache.write(move |result| {
      Ok(result.unpoisoned().delete_threads(&ids))
    })
    .and_then(move |ids| this.message_cache.write(move |result|
      Ok(result.unpoisoned().delete_messages(&ids))
    )).into_box()
  ...
  fn delete_all_threads(&self, mailbox_id: u64) -> MailboxFuture<'a, ()> {
    ...
    .and_then(move |ids| this2.message_cache.write(move |result|
      Ok(result.unpoisoned().delete_messages(&ids))
    )).into_box()
  }
}
...
impl<'a> MessageAccessor<'a> for Arc<MemoryAccessor> {
  ...
  ) -> MailboxFuture<'a, Message>
  {
    ...
    let this = self.clone();
    self.message_cache.write(move |result| {
      result.unpoisoned().put_message(message)
    })
    .and_then(move |message| this.message_thread_cache.write(move |result|
  ...
  fn get_all_messages(&self, thread_id: u64) -> MailboxFuture<'a, Vec<Message>> {
    let this = self.clone();
    self.message_thread_cache.read(move |result| {
      result.unpoisoned().get_thread_by_id(thread_id)
        .ok_or_else(|| MailboxError::not_found("thread id", thread_id))
    })
    .and_then(move |thread| this.message_cache.read(move |result|
      Ok(result
        .unpoisoned()
  ...
  fn delete_message(&self, id: u64) -> MailboxFuture<'a, ()> {
    self.message_cache.write(move |result| {
      Ok(result.unpoisoned().delete_messages(&[id]))
    }).into_box()
  }
  ...
  fn delete_all_messages(&self, thread_id: u64) -> MailboxFuture<'a, ()> {
    ...
    .and_then(move |thread| this.message_cache.write(move |result|
