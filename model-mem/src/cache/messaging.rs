...
impl MailboxCache {
  ...
  pub fn put_mailbox(&mut self, mailbox: Mailbox) -> Result<Mailbox, MessagingError> {
    ...
    match (name_map_entry, id_map_entry) {
      (_, BTEntry::Occupied(_)) => Err(MessagingError::already_exists("Mailbox ID", mailbox.id())),
      (BTEntry::Occupied(_), _) => {
        Err(MessagingError::already_exists("Mailbox name", mailbox.name()))
      }
  ...
  pub fn delete_mailbox_for_owner(
    ...
    let mut name_map_entry = match self.mailbox_owner_map.entry(owner.clone()) {
      HEntry::Occupied(e) => e,
      HEntry::Vacant(_) => return Err(MessagingError::not_found("entry for owner", owner)),
    };
    let result = if let BTEntry::Occupied(e) = name_map_entry.get_mut().entry(String::from(name)) {
      ...
      self
        ...
        .ok_or_else(|| MessagingError::not_found("mailbox id", id))
    } else {
      return Err(MessagingError::not_found("mailbox name map entry", name));
    };
  ...
  pub fn delete_mailbox_by_id(&mut self, id: u64) -> Result<Vec<u64>, MessagingError> {
    let mailbox = match self.mailboxes.remove(&id) {
      ...
      None => {
        ...
        return Err(MessagingError::not_found("mailbox id", id));
      }
    };
    trace!(
      "Deleted mailbox #{} ({} for {})",
  ...
  pub fn delete_all_mailboxes(&mut self, owner: Target) -> Result<Vec<u64>, MessagingError> {
    ...
    let name_map_entry = match self.mailbox_owner_map.entry(owner.clone()) {
      HEntry::Occupied(e) => e,
      HEntry::Vacant(_) => {
        return Err(MessagingError::not_found("name map entry", owner));
      }
    };
...
impl MessageThreadCache {
  ...
  pub fn put_thread(&mut self, thread: MessageThread) -> Result<MessageThread, MessagingError> {
    match self.threads.entry(thread.id) {
      BTEntry::Occupied(_) => Err(MessagingError::already_exists("thread id", thread.id)),
      BTEntry::Vacant(e) => {
...
impl MessageCache {
  ...
  pub fn put_message(&mut self, message: Message) -> Result<Message, MessagingError> {
    match self.messages.entry(message.id) {
      BTEntry::Occupied(_) => Err(MessagingError::already_exists("message id", message.id)),
      BTEntry::Vacant(e) => {
